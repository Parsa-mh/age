  ...
  if (checkbox.checked) {
    ...
    try {
      if (userBirth[0] == "" || userBirth[1] == "" || userBirth[2] == "")
        throw "please fill fields";
      if (userBirth.length != 3) throw "you should enter 3 values";
      if (
        isNaN(Number(userBirth[0])) ||
        isNaN(Number(userBirth[1])) ||
        isNaN(Number(userBirth[2]))
      )
        throw "value is not number";
      if (userBirth[0].length != 4)
        throw "year must be completly with 4 number";
    ...
    try {
      ...
      if (
        Number(userBirth[0]) > NumberYear ||
        NumberYear - Number(userBirth[0]) > 200
      ) {
        swal.fire({
          icon: "error",
          title: "this year is not vailed",
